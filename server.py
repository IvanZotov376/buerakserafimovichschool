...
def index_page():
    """Открывает личный кабинет прямо с Flask-сервера."""
    return send_from_directory(os.path.dirname(os.path.abspath(__file__)), "lk.html")
...
def api_login():
    ...
    if user:
        if password and user.get("password") and user["password"] != password:
        ...
        db.execute(
            "INSERT INTO users (login, password, role, full_name) VALUES (?, ?, 'Ученик', ?)",
            (user_login, password or "123", user_login)
        )
        db.commit()
        return jsonify({"success": True, "user": {"login": user_login, "role": "Ученик", "full_name": user_login, "profile_photo": ""}})
...
def register():
    ...
    if existing:
        db.execute("UPDATE users SET role = ?, full_name = ?, avatar_color = ?, profile_photo = ? WHERE login = ?", (role, full_name or login, avatar_color, profile_photo, login))
    else:
        db.execute("INSERT INTO users (login, password, role, full_name, avatar_color, profile_photo) VALUES (?, '123', ?, ?, ?, ?)", (login, role, full_name or login, avatar_color, profile_photo))
...
def diary_api():
    ...
    school = "МКОУ школа №1 г.Серафимовича"
